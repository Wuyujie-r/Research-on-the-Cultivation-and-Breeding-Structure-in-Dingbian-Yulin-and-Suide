...
# ------------------------------ 2. 设置路径 -----------------------------------
input_path <- "调查问卷初始数据"   # 此处填写初始数据文件路径
output_dir <- "数据清洗结果"  #此处填写输出文件路径
...
# 6.9.1 收入变化趋势
income_trend_col <- "6._家庭收入变化趋势_近五年_:"
if (income_trend_col %in% names(df)) {
  df$income_trend <- df[[income_trend_col]]   # 保留原始文本
  # 编码：1=逐年上升，2=基本稳定，3=波动明显，4=逐年下降
  df$income_trend_code <- case_when(
    grepl("逐年上升", df[[income_trend_col]]) ~ 1,
    grepl("基本稳定", df[[income_trend_col]]) ~ 2,
    grepl("波动明显", df[[income_trend_col]]) ~ 3,
    grepl("逐年下降", df[[income_trend_col]]) ~ 4,
    TRUE ~ NA_real_
  )
  # 是否不稳定（波动或下降）
  df$income_unstable <- ifelse(df$income_trend_code %in% c(3,4), 1, 0)
} else {
  warning("未找到收入变化趋势列")
  df$income_trend <- NA
  df$income_trend_code <- NA
  df$income_unstable <- NA
}

# 6.9.2 近三年是否遭遇生产风险
risk_col <- "33.1._近三年内您是否遭遇过农业或畜牧生产风险_如自然灾害、疫病、价格大跌等"
if (risk_col %in% names(df)) {
  df$risk_encounter <- ifelse(grepl("是", df[[risk_col]]), 1, 0)
} else {
  warning("未找到生产风险列")
  df$risk_encounter <- NA
}

# 6.9.3 是否考虑调整种养结构
adjust_col <- "34._若农业或畜牧收益不稳定，您是否考虑过调整种养结构"
if (adjust_col %in% names(df)) {
  df$consider_adjust <- ifelse(grepl("是", df[[adjust_col]]), 1, 0)
} else {
  warning("未找到调整种养结构列")
  df$consider_adjust <- NA
}

# 6.9.4 常年外出务工人数
migrant_col <- "2_人;_其中，常年外出务工人数_6个月以上_:"
if (migrant_col %in% names(df)) {
  df$long_migrant_num <- safe_as_numeric(df[[migrant_col]])
  df$long_migrant_num[is.na(df$long_migrant_num)] <- 0
  df$long_migrant <- ifelse(df$long_migrant_num > 0, 1, 0)
} else {
  warning("未找到常年外出务工人数列")
  df$long_migrant_num <- 0
  df$long_migrant <- 0
}

# 注：其他风险感知（养殖风险感知、种植风险感知、未来前景判断）问卷中无直接对应，留空
df$risk_livestock <- NA
df$risk_crop <- NA
df$future_outlook <- NA
df$peer_behavior <- NA
...
missing_stats <- data.frame(
  变量 = c("gender", "age", "edu_level", "labor_num", "cultivated_area",
         ...
         "income_livestock_share", "changed_industry", "income_trend", "risk_encounter"),
...
core_vars <- c(
  ...
  # 认知感知（新增）
  "income_trend", "income_trend_code", "income_unstable",
  "risk_encounter", "consider_adjust",
  "long_migrant_num", "long_migrant",
  "risk_livestock", "risk_crop", "future_outlook", "peer_behavior",
  # 政策
...
report_lines <- c(
  ...
  "",
  "六、新增认知感知变量（第五章用）",
  paste("收入不稳定比例:", round(mean(df$income_unstable, na.rm = TRUE) * 100, 1), "%"),
  paste("遭遇生产风险比例:", round(mean(df$risk_encounter, na.rm = TRUE) * 100, 1), "%"),
  paste("考虑调整种养结构比例:", round(mean(df$consider_adjust, na.rm = TRUE) * 100, 1), "%"),
  paste("有常年外出务工比例:", round(mean(df$long_migrant, na.rm = TRUE) * 100, 1), "%")
)
